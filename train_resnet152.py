...
def classifier_train_with_constraint(mode=None):
    ...
    res152 = ResNet_152(feature_out=True,vector_out=True,dim_out=para.classNums)

   
    for idx,layer in enumerate(res152.layers):
    ...
    weights_save_dir = "./ckpt/classifier-resnet152-99-constrainted-by-" + weights_dir_dict[mode] + "/" + "classifier-resnet152-99-constrainted-by-" + weights_dir_dict[mode]
    
    res152.load_weights("./ckpt/classifier-resnet152-99-40-retrain/classifier-resnet152-99-40-retrain")
    ...
    for epoch in range(para.constraint_epochs):
        ...
        for idx,(img,lab_mask,lab) in process:
            ...
            with tf.GradientTape(persistent=True) as tape:
                res152.feature_out = True
                res152.vector_out = True
                (preds,convOuts,hiddenout) = res152(img)  # preds after softmax
            ...
            if ((idx+1) % para.fine_tune_batch_size == 0):
                gd = tape.gradient(batch_loss,res152.trainable_variables)
                opt.apply_gradients(zip(gd,res152.trainable_variables))
                batch_loss = tf.constant(0,dtype=tf.float32)
        ...
        if (global_loss > mean_loss.numpy()):
            ...
            res152.save_weights(weights_save_dir)
        ...
        for ts_idx,(ts_img,ts_labmask,ts_lab) in ts_process:
            ...
            with tf.GradientTape() as tape:
                res152.feature_out = True
                res152.vector_out = False
                _,cam_mask,ts_pred = compute_heatmap(res152,ts_img,np.argmax(ts_lab.squeeze(axis=0)),size_holder,tape=tape,training=False)
...
def classifier_train_without_constraint():
    ...
    res152 = ResNet_152(feature_out=False,dim_out=classNums)
    
    # param block
    # first 20 eps block all resnet152's layers(only train classfier head),sec 10 eps block only resnet152's pre-122 layers,final 10 eps train all trainable layers
    ...
    weights_save_dir = "./ckpt/classifier-resnet152-99-20-retrain/classifier-resnet152-99-20-retrain"

    # weights_save_dir = "./ckpt/classifier-resnet152-99-30-retrain/classifier-resnet152-99-30-retrain"
    # res152.load_weights("./ckpt/classifier-resnet152-99-20-retrain/classifier-resnet152-99-20-retrain")

    # weights_save_dir = "./ckpt/classifier-resnet152-99-40-retrain/classifier-resnet152-99-40-retrain"
    # res152.load_weights("./ckpt/classifier-resnet152-99-30-retrain/classifier-resnet152-99-30-retrain")

    # weights_save_dir = "./ckpt/classifier-resnet152-99-60-retrain/classifier-resnet152-99-60-retrain"
    # res152.load_weights("./ckpt/classifier-resnet152-99-40-retrain/classifier-resnet152-99-40-retrain")
...
def metrics_test():
    ...
    res152 = ResNet_152(feature_out=True,dim_out=para.classNums)

    weights_save_dir = "./ckpt/classifier-resnet152-99-60-retrain/classifier-resnet152-99-60-retrain"
    # weights_save_dir = "./ckpt/classifier-resnet152-99-constrainted-by-labelmask/classifier-resnet152-99-constrainted-by-labelmask"
    # weights_save_dir = "./ckpt/classifier-resnet152-99-constrainted-by-instancemask/classifier-resnet152-99-constrainted-by-instancemask"
    # weights_save_dir = "./ckpt/classifier-resnet152-99-constrainted-by-componentmask/classifier-resnet152-99-constrainted-by-componentmask"
    # weights_save_dir = "./ckpt/classifier-resnet152-99-constrainted-by-knowledge-inject/classifier-resnet152-99-constrainted-by-knowledge-inject"
    # weights_save_dir = "./ckpt/classifier-resnet152-99-constrainted-by-labelmask-union/classifier-resnet152-99-constrainted-by-labelmask-union"
    # weights_save_dir = "./ckpt/classifier-resnet152-99-constrainted-by-instancemask-union/classifier-resnet152-99-constrainted-by-instancemask-union"
    # weights_save_dir = "./ckpt/classifier-resnet152-99-constrainted-by-componentmask-union/classifier-resnet152-99-constrainted-by-componentmask-union"

    res152.load_weights(weights_save_dir)
    
    res_dir = './result/resnet152-baseline-60'
    # res_dir = './result/resnet152-labelmask'
    # res_dir = './result/resnet152-instancemask'
    # res_dir = './result/resnet152-componentmask'
    # res_dir = './result/resnet152-knowledge-inject'
    # res_dir = './result/resnet152-labelmask-union'
    # res_dir = './result/resnet152-instancemask-union'
    # res_dir = './result/resnet152-componentmask-union'
    ...
    for idx,(img,lab_mask,lab) in process:
        ...
        with tf.GradientTape() as t:
            cam3,cam_mask,preds = compute_heatmap(res152,img,np.argmax(lab.squeeze(axis=0)),size_holder,tape=t,training=False)
...
def main():
    ...
    # first 40 epochs: first 20 eps block all resnet152's layers(only train classfier head),sec 10 eps block only resnet152's pre-xx layers,final 10 eps train all trainable layers
