...
def classifier_train_with_constraint(mode=None):
    ...
    res101 = ResNet_101(feature_out=True,vector_out=True,dim_out=para.classNums)

   
    for idx,layer in enumerate(res101.layers):
    ...
    weights_save_dir = "./ckpt/classifier-resnet101-99-constrainted-by-" + weights_dir_dict[mode] + "/" + "classifier-resnet101-99-constrainted-by-" + weights_dir_dict[mode]
    
    res101.load_weights("./ckpt/classifier-resnet101-99-40-retrain/classifier-resnet101-99-40-retrain")
    ...
    for epoch in range(para.constraint_epochs):
        ...
        for idx,(img,lab_mask,lab) in process:
            ...
            lab_mask = cv.resize(lab_mask,(int(size_holder[0]/32),int(size_holder[0]/32)))
            ...
            with tf.GradientTape(persistent=True) as tape:
                res101.feature_out = True
                res101.vector_out = True
                (preds,convOuts,hiddenout) = res101(img)  # preds after softmax
 
                if "KI" in mode:
                    # Align Loss
                    classidx = np.argmax(lab[0])
                    knowledge = np.loadtxt('./txt-99/{}.txt'.format(classidx))
            ...
            if ((idx+1) % para.fine_tune_batch_size == 0):
                gd = tape.gradient(batch_loss,res101.trainable_variables)
                opt.apply_gradients(zip(gd,res101.trainable_variables))
                batch_loss = tf.constant(0,dtype=tf.float32)
        ...
        if (global_loss > mean_loss.numpy()):
            ...
            res101.save_weights(weights_save_dir)
        ...
        for ts_idx,(ts_img,ts_labmask,ts_lab) in ts_process:
            ...
            with tf.GradientTape() as tape:
                res101.feature_out = True
                res101.vector_out = False
                _,cam_mask,ts_pred = compute_heatmap(res101,ts_img,np.argmax(ts_lab.squeeze(axis=0)),size_holder,tape=tape,training=False)
...
def classifier_train_without_constraint():
    ...
    res101 = ResNet_101(feature_out=False,dim_out=classNums)
    
    # param block
    # first 20 eps block all resnet101's layers(only train classfier head),sec 10 eps block only resnet101's pre-122 layers,final 10 eps train all trainable layers
    
    # for example,first 20 eps will be like the following
    for idx,layer in enumerate(res101.layers):  
        if ('resnet' in layer.name):
            for layer_ in layer.layers[:122]:
    ...
    weights_save_dir = "./ckpt/classifier-resnet101-99-20-retrain/classifier-resnet101-99-20-retrain"

    # weights_save_dir = "./ckpt/classifier-resnet101-99-30-retrain/classifier-resnet101-99-30-retrain"
    # res101.load_weights("./ckpt/classifier-resnet101-99-20-retrain/classifier-resnet101-99-20-retrain")

    # weights_save_dir = "./ckpt/classifier-resnet101-99-40-retrain/classifier-resnet101-99-40-retrain"
    # res101.load_weights("./ckpt/classifier-resnet101-99-30-retrain/classifier-resnet101-99-30-retrain")

    # weights_save_dir = "./ckpt/classifier-resnet101-99-60-retrain/classifier-resnet101-99-60-retrain"
    # res101.load_weights("./ckpt/classifier-resnet101-99-40-retrain/classifier-resnet101-99-40-retrain")
    ...
    for epoch in range(20):
        ...
        # first 20 epochs: base_linear_lr, second 10 epochs:base_opt_convlast_lr, final 10 eps: base_opt_convall_lr
...
def metrics_test():
    ...
    res101 = ResNet_101(feature_out=True,dim_out=para.classNums)

    weights_save_dir = "./ckpt/classifier-resnet101-99-60-retrain/classifier-resnet101-99-60-retrain"
    # weights_save_dir = "./ckpt/classifier-resnet101-99-constrainted-by-labelmask/classifier-resnet101-99-constrainted-by-labelmask"
    # weights_save_dir = "./ckpt/classifier-resnet101-99-constrainted-by-instancemask/classifier-resnet101-99-constrainted-by-instancemask"
    # weights_save_dir = "./ckpt/classifier-resnet101-99-constrainted-by-componentmask/classifier-resnet101-99-constrainted-by-componentmask"
    # weights_save_dir = "./ckpt/classifier-resnet101-99-constrainted-by-knowledge-inject/classifier-resnet101-99-constrainted-by-knowledge-inject"
    # weights_save_dir = "./ckpt/classifier-resnet101-99-constrainted-by-labelmask-union/classifier-resnet101-99-constrainted-by-labelmask-union"
    # weights_save_dir = "./ckpt/classifier-resnet101-99-constrainted-by-instancemask-union/classifier-resnet101-99-constrainted-by-instancemask-union"
    # weights_save_dir = "./ckpt/classifier-resnet101-99-constrainted-by-componentmask-union/classifier-resnet101-99-constrainted-by-componentmask-union"

    res101.load_weights(weights_save_dir)
    
    res_dir = './result/resnet101-baseline-60'
    # res_dir = './result/resnet101-labelmask'
    # res_dir = './result/resnet101-instancemask'
    # res_dir = './result/resnet101-componentmask'
    # res_dir = './result/resnet101-knowledge-inject'
    # res_dir = './result/resnet101-labelmask-union'
    # res_dir = './result/resnet101-instancemask-union'
    # res_dir = './result/resnet101-componentmask-union'
    ...
    for idx,(img,lab_mask,lab) in process:
        imgcam = img/255.0
        imgcam = np.expand_dims(imgcam,axis=0)
        ...
        with tf.GradientTape() as t:
            cam3,cam_mask,preds = compute_heatmap(res101,img,np.argmax(lab.squeeze(axis=0)),size_holder,tape=t,training=False)
            cam = np.where(cam3 > np.percentile(cam3, 100 - keep_percent), 1, 0)
            cam_mask = cam_mask.numpy().squeeze()
            cam_mask = cv.resize(cam_mask,size_holder)
            seg,heatmap = overlay_gradCAM(imgcam.squeeze(axis=0),cam3=cam3,cam=cam)
...
def main():
    ...
    # first 40 epochs: first 20 eps block all resnet101's layers(only train classfier head),sec 10 eps block only resnet101's pre-xx layers,final 10 eps train all trainable layers
    # 40--60 epochs: After training the model for 40 epochs, further train it for 20 epochs using only accuracy loss, to compare with the guided trained model
