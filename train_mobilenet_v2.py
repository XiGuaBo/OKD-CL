...
def classifier_train_with_constraint(mode = None):
    ...
    # model
    v2 = MobileNet_v2(feature_out=True,vector_out=True,dim_out=para.classNums)
    
    for idx,layer in enumerate(v2.layers):
    ...
    weights_save_dir = "./ckpt/classifier-mobilenet_v2-99-constrainted-by-" + weights_dir_dict[mode] + "/" + "classifier-mobilenet_v2-99-constrainted-by-" + weights_dir_dict[mode]
    
    v2.load_weights("./ckpt/classifier-mobilenet_v2-99-40-retrain/classifier-mobilenet_v2-99-40-retrain")
    global_loss = UNLIMIT
    ...
    for epoch in range(para.constraint_epochs):
        ...
        for idx,(img,lab_mask,lab) in process:
            ...
            with tf.GradientTape(persistent=True) as tape:
                v2.feature_out = True
                v2.vector_out = True
                (preds ,convOuts,hiddenout) = v2(img)  # preds after softmax
            ...
            if ((idx+1) % para.fine_tune_batch_size == 0):
                gd = tape.gradient(batch_loss,v2.trainable_variables)
                # w_gd = tape.gradient(batch_loss,[w_acc,w_am])
                opt.apply_gradients(zip(gd,v2.trainable_variables))
        ...
        if (global_loss > mean_loss.numpy()):
            ...
            v2.save_weights(weights_save_dir)
        ...
        for ts_idx,(ts_img,ts_labmask,ts_lab) in ts_process:
            ...
            with tf.GradientTape() as tape:
                v2.feature_out = True
                v2.vector_out = False
                _,cam_mask,ts_pred = compute_heatmap(v2,ts_img,np.argmax(ts_lab.squeeze(axis=0)),size_holder,tape=tape,training=False)
...
def classifier_train_without_constraint():
    ...
    v2 = MobileNet_v2(feature_out=False,dim_out=classNums)
    
    # param block
    # first 20 eps block all mobilenetv2's layers(only train classfier head),sec 10 eps block only mobilenetv2's pre-98 layers,final 10 eps train all trainable layers
    
    # for example,first 20 eps will be like the following
    for idx,layer in enumerate(v2.layers):
        if ('mobilenet' in layer.name):
            for layer_ in layer.layers[:98]:
    ...
    weights_save_dir = "./ckpt/classifier-mobilenet_v2-99-20-retrain/classifier-mobilenet_v2-99-20-retrain"

    # weights_save_dir = "./ckpt/classifier-mobilenet_v2-99-30-retrain/classifier-mobilenet_v2-99-30-retrain"
    # v2.load_weights("./ckpt/classifier-mobilenet_v2-99-20-retrain/classifier-mobilenet_v2-99-20-retrain")

    # weights_save_dir = "./ckpt/classifier-mobilenet_v2-99-40-retrain/classifier-mobilenet_v2-99-40-retrain"
    # v2.load_weights("./ckpt/classifier-mobilenet_v2-99-30-retrain/classifier-mobilenet_v2-99-30-retrain")
...
def metrics_test():
    ...
    v2 = MobileNet_v2(feature_out=True,dim_out=classNums)

    weights_save_dir = "./ckpt/classifier-mobilenet_v2-99-60-retrain/classifier-mobilenet_v2-99-60-retrain"
    # weights_save_dir = "./ckpt/classifier-mobilenet_v2-99-constrainted-by-labelmask/classifier-mobilenet_v2-99-constrainted-by-labelmask"
    # weights_save_dir = "./ckpt/classifier-mobilenet_v2-99-constrainted-by-instancemask/classifier-mobilenet_v2-99-constrainted-by-instancemask"
    # weights_save_dir = "./ckpt/classifier-mobilenet_v2-99-constrainted-by-componentmask/classifier-mobilenet_v2-99-constrainted-by-componentmask"
    # weights_save_dir = "./ckpt/classifier-mobilenet_v2-99-constrainted-by-knowledge-inject/classifier-mobilenet_v2-99-constrainted-by-knowledge-inject"
    ...
    res_dir = './result/mobilenet_v2-baseline-60'
    # res_dir = './result/mobilenet_v2-labelmask'
    # res_dir = './result/mobilenet_v2-instancemask'
    # res_dir = './result/mobilenet_v2-componentmask'
    # res_dir = './result/mobilenet_v2-knowledge-inject'
    # res_dir = './result/mobilenet_v2-labelmask-union'
    # res_dir = './result/mobilenet_v2-instancemask-union'
    # res_dir = './result/mobilenet_v2-componentmask-union'
...
def main():
    ...
    # first 40 epochs: first 20 eps block all mobilenetv2's layers(only train classfier head),sec 10 eps block only mobilenetv2's pre-xx layers,final 10 eps train all trainable layers
