...
def classifier_train_with_constraint(mode = None):
    ...
    # model
    inceptionv3 = Inception_V3(feature_out=True,vector_out=True,dim_out=para.classNums)
    
    for idx,layer in enumerate(inceptionv3.layers):
    ...
    weights_save_dir = "./ckpt/classifier-inception_v3-99-constrainted-by-" + weights_dir_dict[mode] + "/" + "classifier-inception_v3-99-constrainted-by-" + weights_dir_dict[mode]

    inceptionv3.load_weights("./ckpt/classifier-inception_v3-99-40-retrain/classifier-inception_v3-99-40-retrain")
    ...
    for epoch in range(para.constraint_epochs):
        ...
        for idx,(img,lab_mask,lab) in process:
            ...
            lab_mask = cv.resize(lab_mask,(5,5))
            ...
            with tf.GradientTape(persistent=True) as tape:
                inceptionv3.feature_out = True
                inceptionv3.vector_out = True
                (preds ,convOuts,hiddenout) = inceptionv3(img)
            ...
            if ((idx+1) % para.fine_tune_batch_size == 0):
                gd = tape.gradient(batch_loss,inceptionv3.trainable_variables)
                opt.apply_gradients(zip(gd,inceptionv3.trainable_variables))
                batch_loss = tf.constant(0,dtype=tf.float32)
        ...
        if (global_loss > mean_loss.numpy()):
            ...
            inceptionv3.save_weights(weights_save_dir)
        ...
        for ts_idx,(ts_img,ts_labmask,ts_lab) in ts_process:
            ...
            with tf.GradientTape() as tape:
                inceptionv3.feature_out = True
                inceptionv3.vector_out = False
                _,cam_mask,ts_pred = compute_heatmap(inceptionv3,ts_img,np.argmax(ts_lab.squeeze(axis=0)),size_holder,tape=tape,training=False)
...
def classifier_train_without_constraint():
    ...
    inceptionv3 = Inception_V3(feature_out=False,dim_out=classNums)
    
    # param block
    # first 20 eps block all inceptionv3's layers(only train classfier head),sec 10 eps block only inceptionv3's pre-249 layers,final 10 eps train all trainable layers
    
    # for example,first 20 eps will be like the following
    for idx,layer in enumerate(inceptionv3.layers):
        if ('inception_v3' in layer.name):
            for layer_ in layer.layers[:249]:
    ...
    weights_save_dir = "./ckpt/classifier-inception_v3-99-20-retrain/classifier-inception_v3-99-20-retrain"

    # weights_save_dir = "./ckpt/classifier-inception_v3-99-30-retrain/classifier-inception_v3-99-30-retrain"
    # inceptionv3.load_weights("./ckpt/classifier-inception_v3-99-20-retrain/classifier-inception_v3-99-20-retrain")

    # weights_save_dir = "./ckpt/classifier-inception_v3-99-40-retrain/classifier-inception_v3-99-40-retrain"
    # inceptionv3.load_weights("./ckpt/classifier-inception_v3-99-30-retrain/classifier-inception_v3-99-30-retrain")

    # weights_save_dir = "./ckpt/classifier-inception_v3-99-60-retrain/classifier-inception_v3-99-60-retrain"
    # inceptionv3.load_weights("./ckpt/classifier-inception_v3-99-40-retrain/classifier-inception_v3-99-40-retrain")
    global_loss = UNLIMIT
...
def metrics_test():
    ...
    inceptionv3 = Inception_V3(feature_out=True,dim_out=classNums)
    
    weights_save_dir = "./ckpt/classifier-inception_v3-99-60-retrain/classifier-inception_v3-99-60-retrain"
    # weights_save_dir = "./ckpt/classifier-inception_v3-99-constrainted-by-labelmask/classifier-inception_v3-99-constrainted-by-labelmask"
    # weights_save_dir = "./ckpt/classifier-inception_v3-99-constrainted-by-instancemask/classifier-inception_v3-99-constrainted-by-instancemask"
    # weights_save_dir = "./ckpt/classifier-inception_v3-99-constrainted-by-componentmask/classifier-inception_v3-99-constrainted-by-componentmask"
    # weights_save_dir = "./ckpt/classifier-inception_v3-99-constrainted-by-knowledge-inject/classifier-inception_v3-99-constrainted-by-knowledge-inject"
    # weights_save_dir = "./ckpt/classifier-inception_v3-99-constrainted-by-labelmask-union/classifier-inception_v3-99-constrainted-by-labelmask-union"
    # weights_save_dir = "./ckpt/classifier-inception_v3-99-constrainted-by-instancemask-union/classifier-inception_v3-99-constrainted-by-instancemask-union"
    # weights_save_dir = "./ckpt/classifier-inception_v3-99-constrainted-by-componentmask-union/classifier-inception_v3-99-constrainted-by-componentmask-union"
    
    inceptionv3.load_weights(weights_save_dir)
    
    res_dir = './result/inception_v3-baseline-60'
    # res_dir = './result/inception_v3-labelmask'
    # res_dir = './result/inception_v3-instancemask'
    # res_dir = './result/inception_v3-componentmask'
    # res_dir = './result/inception_v3-knowledge-inject'
    # res_dir = './result/inception_v3-labelmask-union'
    # res_dir = './result/inception_v3-instancemask-union'
    # res_dir = './result/inception_v3-componentmask-union'
...
def main():
    ...
    # first 40 epochs: first 20 eps block all inceptionv3's layers(only train classfier head),sec 10 eps block only inceptionv3's pre-xx layers,final 10 eps train all trainable layers
